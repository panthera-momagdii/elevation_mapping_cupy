...
def map_utils(
    ...
    util_preamble = string.Template(
        """
        // Upstream used `float` literally as a CUDA type, but cupy 14
        // forward-declares it as an incomplete placeholder. Map data is
        // actually np.float32, so use plain `float` everywhere here.
